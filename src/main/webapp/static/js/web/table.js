define(['jquery'],
function($) {
    $(document).on('change', 'table tfoot [type="checkbox"]',
    function(e) {
        e && e.preventDefault();
    ...
    $(document).on('change', '.page-table-header input[type="checkbox"]',
    function(e) {
        e && e.preventDefault();
...
//            var selecteds = get_selecteds();
            var selecteds = $('tbody tr td:first-child input[name=\'ids\']:enabled:checked');
            var submit = function() {
                buttontype == 'button' ? $this.html('<i class="fa fa-spinner fa-spin"></i> ' + tip.lang.processing) : $this.val(tip.lang.processing);
                $.post(href, /*{ids: selecteds}*/selecteds.serialize()).done(function(data) {
//                    data = eval("(" + data + ")");
//                    if (data.status == 1) {
...
//                            tip.msgbox.suc(data.result.message || tip.lang.success, data.result.url)
                            tip.msgbox.suc(data.msg || tip.lang.success, data.data.url)
                        }
                        buttontype == 'button' ? $this.html(html) : $this.val(html)
                    } else {
                        buttontype == 'button' ? $this.html(html) : $this.val(html);
//                        tip.msgbox.err(data.result.message || tip.lang.error)
                        tip.msgbox.err(data.msg || tip.lang.error)
                    }
                }).fail(function() {
                    buttontype == 'button' ? $this.html(html) : $this.val(html),
                    tip.msgbox.err(tip.lang.exception)
        ...
        function(e) {
            ...
            checked = $(e.target).is(":checked");
            $('.page-table-header input[type="checkbox"]', t).prop("checked", checked),
        ...
        $(document).on("change", '.table-responsive tfoot td:first [type="checkbox"]',
        function(e) {
            e && e.preventDefault();
