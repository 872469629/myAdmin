...
<ul class="pagination pagination-centered">
	[#list segment as segmentPageNumber]
		...
		[#if segmentPageNumber != pageNumber]
			<li>
				<a href="javascript: $.pageSkip('${url}',${segmentPageNumber});" >${segmentPageNumber}</a>
			</li>
		[#else]
	...
	[#if hasNext]
		<li>
			<a href="javascript: $.pageSkip('${url}',${nextPageNumber});"  class="pager-nav">下一页»</a>
		</li>
	[/#if]
	[#if isLast]
		<li>
			<a href="javascript: $.pageSkip('${url}',${lastPageNumber});"  class="pager-nav">尾页</a>
		</li>
	[/#if]
</ul>
[/#if]
